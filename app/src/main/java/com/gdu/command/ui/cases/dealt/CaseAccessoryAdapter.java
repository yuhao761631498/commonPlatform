 ...
 * 案件取证视频列表
 */
public class CaseAccessoryAdapter extends BaseAdapter {

    private Context mContext;
    private List<String> mAccessoryPathList;

    public CaseAccessoryAdapter(Context context) {
        mContext = context;
    }

    public void setData(List<String> pathList) {
        mAccessoryPathList = pathList;
    ...
    @Override
    public int getCount() {
        if (mAccessoryPathList != null) {
            return mAccessoryPathList.size();
        }
        return 0;
    }
    ...
    public Object getItem(int position) {
        if (mAccessoryPathList != null) {
            return mAccessoryPathList.get(position);
        }
        return null;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_case_accessory, null);
            holder.mCaseImageView = convertView.findViewById(R.id.case_imageview);
            holder.mPlayVideoImageView = convertView.findViewById(R.id.play_video_imageview);
        ...
        String casePath = mAccessoryPathList.get(position);
        String path = casePath;
        if (!casePath.contains("http:")) {
            path = "file://" + casePath;
        }
        holder.mPlayVideoImageView.setVisibility(View.GONE);
        if (path.contains("mp4")) {
            loadCover(holder.mCaseImageView, path);
            holder.mPlayVideoImageView.setVisibility(View.VISIBLE);
        } else {
