...
public class RequestConfig implements Parcelable {
    ...
    public boolean isTakeVideo = false;  //调用相机拍照
    public boolean isTakePhoto = true;   //调用相机录像
    public boolean isSingle = false; // 是否单选
    public boolean isContainVideo = false; //是否包含视频
    public boolean isContainPhoto = false; //是否包含图片
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeByte(this.isContainVideo ? (byte) 1 : (byte) 0);
        dest.writeByte(this.isContainPhoto ? (byte) 1 : (byte) 0);
        dest.writeByte(this.isTakePhoto ? (byte) 1 : (byte) 0);
        dest.writeByte(this.isTakeVideo ? (byte) 1 : (byte) 0);
    ...
    protected RequestConfig(Parcel in) {
        ...
        this.isContainVideo = in.readByte() != 0;
        this.isContainPhoto = in.readByte() != 0;
        this.isTakePhoto = in.readByte() != 0;
        this.isTakeVideo = in.readByte() != 0;
